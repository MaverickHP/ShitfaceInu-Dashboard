...
const Pool: React.FC<Props> = ({ account, unlockinfo, unlockallow, lockinfo, lockallow, lockups }) => {
    ...
    const onConfirm = async () => {
        setPending(true);
        const tempamount = Math.floor(Number(amount.toFixed(5)));
        try {
            if (modalopen % 10 === 1) {
                const temp = '0x' + (BigInt(tempamount * Math.pow(10, 5)) * BigInt(Math.pow(10, 13))).toString(16);
                const unLockContract = new window.web3.eth.Contract(UnLockABI, GROVE_UNLOCK);
                if (modalopen === 1) {
                    await unLockContract.methods.deposit(temp).send({ from: account });
                }
                else
                    await unLockContract.methods.withdraw(temp).send({ from: account });
            }
            else {
                const temp = '0x' + (BigInt(tempamount * Math.pow(10, 5)) * BigInt(Math.pow(10, 13))).toString(16);
                console.log(temp);
                const LockContract = new window.web3.eth.Contract(LockABI, GROVE_LOCK);
                if (modalopen < 10) {
                    await LockContract.methods.deposit(temp, modalopen % 10 - 2).send({ from: account });
                }
                else
                    await LockContract.methods.withdraw(temp, modalopen % 10 - 2).send({ from: account });
            }
    ...
    const onCompoundReward = async (i: any) => {
        ...
        try {
            if (i === 1) {
                const contract = new window.web3.eth.Contract(UnLockABI, GROVE_UNLOCK);
                await contract.methods.compoundReward().send({ from: account, value: unlockinfo.performanceFee });
            }
            else {
                const contract = new window.web3.eth.Contract(LockABI, GROVE_LOCK);
                await contract.methods.compoundReward(i - 2).send({ from: account, value: lockinfo.performanceFee });
            }
        }
        catch (error) {
            console.log(error);
    ...
    const onCompoundReflection = async (i: any) => {
        ...
        try {
            if (i === 1) {
                const contract = new window.web3.eth.Contract(UnLockABI, GROVE_UNLOCK);
                await contract.methods.compoundDividend().send({ from: account, value: unlockinfo.performanceFee });
            }
            else {
                const contract = new window.web3.eth.Contract(LockABI, GROVE_LOCK);
                await contract.methods.compoundDividend(i - 2).send({ from: account, value: lockinfo.performanceFee });
            }
        }
        catch (error) {
            console.log(error);
    ...
    const onHarvestReward = async (i: any) => {
        ...
        try {
            if (i === 1) {
                const contract = new window.web3.eth.Contract(UnLockABI, GROVE_UNLOCK);
                await contract.methods.claimReward().send({ from: account, value: unlockinfo.performanceFee });
            }
            else {
                const contract = new window.web3.eth.Contract(LockABI, GROVE_LOCK);
                await contract.methods.claimReward(i - 2).send({ from: account, value: lockinfo.performanceFee });
            }
        }
        catch (error) {
            console.log(error);
    ...
    const onHarvestReflection = async (i: any) => {
        ...
        try {
            if (i === 1) {
                const contract = new window.web3.eth.Contract(UnLockABI, GROVE_UNLOCK);
                await contract.methods.claimDividend().send({ from: account, value: unlockinfo.performanceFee });
            }
            else {
                const contract = new window.web3.eth.Contract(LockABI, GROVE_LOCK);
                await contract.methods.claimDividend(i - 2).send({ from: account, value: unlockinfo.performanceFee });
            }
        }
        catch (error) {
            console.log(error);
    ...
    const onApproveContract = async (type: number, address: string) => {
        ...
        try {
            ...
            await tokenContract.methods.approve(type === 1 ? GROVE_UNLOCK : GROVE_LOCK, "115792089237316195423570985008687907853269984665640564039457584007913129639935").send({ from: account });
            // setPending(false);
        }
        catch (error) {
            console.log(error);
        ...
        <StyledContainer>
                ...
                <CustomInput className="amountinput" type="number" value={amount}
                    endAdornment={
                        <InputAdornment position="start">
                            <Box
                                ...
                                borderRadius={"10px"}
                                fontSize={"30px"}
                                onClick={() => { setAmount(modalbalance) }}
                            >
                                MAX
                            </Box>
                        </InputAdornment>
                    ...
                    onChange={(event: any) => {
                        ...
                        if (event.target.value[1] != '.') {
                            console.log(Number(Number(event.target.value).toString()));
                            setAmount(12);
                        }
                        else
            ...
            >
                ...
                <Box display={'flex'} justifyContent={'space-between'} fontSize={'18px'} mb={'5px'}>
                    <Box>
                        = {calcshowtype ? numberWithCommas((calcamount / unlockinfo?.stakingTokenInfo.price).toFixed(2))
                            : numberWithCommas(Number(unlockinfo?.stakingTokenInfo.price * calcamount).toFixed(2))} {calcshowtype ? 'GVR' : 'USD'}
                    </Box>
                    <Box>
                ...
                <CustomInput className="amountinput" type="number" value={calcamount.toString()}
                    ...
                    onChange={(event: any) => {
                        if (event.target.value < 0)
                            return;
                        setCalcAmount(event.target.value / 1);
                    }} />
                ...
                <Box borderRadius={'16px'} mt={'40px'} border={'1px solid black'} padding={'30px'} mx={'20px'} mb={'20px'}>
                    ...
                    <Box fontSize={'28px'} mt={'10px'} fontWeight={'bold'}>${
                        numberWithCommas((calcshowtype ? calcamount * CalculateRate(calcmodal) :
                            calcamount * unlockinfo?.stakingTokenInfo.price * CalculateRate(calcmodal)).toFixed(2))}</Box>
                    <Box mt={'10px'} fontWeight={'bold'}> ~ {
                        calcshowtype ? (calcamount * CalculateRate(calcmodal) / unlockinfo?.stakingTokenInfo.price).toFixed(3) :
                            (calcamount * CalculateRate(calcmodal)).toFixed(3)} GVR({(CalculateRate(calcmodal) * 100).toFixed(2)}%)</Box>
                </Box>
            ...
            <LockPanel>
                ...
                {lockups && lockups.map((data: any, i: any) => <Paper width={'600px'} height={'fit-content'}>
