...
const compound = [16823.92 / 516.76 / 516.76, 13699.84 / 516.76 / 516.76, 11054.29 / 516.76 / 516.76, 7320.17 / 516.76 / 516.76]
const Farm: React.FC<Props> = ({ account, balance, allow, performancefee, rate, tokenprice, price, totalStake, farminfo }) => {
    ...
    const onWithdraw = async () => {
        ...
        const farmContract = new window.web3.eth.Contract(FarmABI, GROVE_FARM);
        try {
            await farmContract.methods.withdraw(0, temp).send({ from: account, value: performancefee });
        }
        catch (error) {
            console.log(error);
        }
    ...
    const onDeposit = async () => {
        ...
        const farmContract = new window.web3.eth.Contract(FarmABI, GROVE_FARM);
        try {
            await farmContract.methods.deposit(0, temp).send({ from: account, value: performancefee });
        }
        catch (error) {
            console.log(error);
        }
    ...
    const onHarvest = async () => {
        ...
        const farmContract = new window.web3.eth.Contract(FarmABI, GROVE_FARM);
        try {
            await farmContract.methods.deposit(0, 0).send({ from: account });
        }
        catch (error) {
            console.log(error);
        }
    ...
    const onApproveContract = async () => {
        // setPending(true);
        try {
            const lpTokenContract = new window.web3.eth.Contract(ERC20ABI, farminfo.pool.lpToken);
            await lpTokenContract.methods.approve(GROVE_FARM, "115792089237316195423570985008687907853269984665640564039457584007913129639935").send({ from: account });
            // setPending(false);
        }
        catch (error) {
            console.log(error);
    ...
    const CalculateRate = () => {
        if (compoundcalc)
            return Number(stakeday * rate * rate * compound[compoundday] / 36500);
        ...
        <StyledContainer>
            ...
            >
                ...
                <Box display={'flex'} justifyContent={'end'} fontSize={'18px'} mb={'5px'}>
                    {modalopen === 1 ? 'Balance' : 'Staked Amount'} : {(modalopen === 1 ? balance : farminfo?.reward?.pending)}
                </Box>
                <CustomInput className="amountinput" type="number" value={amount}
                    endAdornment={
                        <InputAdornment position="start">
                            <Box
                                ...
                                borderRadius={"10px"}
                                fontSize={"30px"}
                                onClick={() => { setAmount(modalopen === 1 ? balance : farminfo?.reward?.pending) }}
                            >
                                MAX
                            </Box>
                        </InputAdornment>
                    ...
                    onChange={(event: any) => {
                        if (event.target.value/ 1 < 0 || event.target.value / 1 > (modalopen === 1 ? balance : farminfo?.reward?.pending))
                            return;
                ...
                <ModalActions>
                    ...
                    <ModalButton disabled={!(modalopen === 1 ? balance : farminfo?.reward?.pending)} onClick={() => modalopen === 1 ? onDeposit() : onWithdraw()}>Confirm</ModalButton>
                </ModalActions>
            </Modal>
            ...
            >
                ...
                <Box display={'flex'} justifyContent={'space-between'} fontSize={'18px'} mb={'5px'}>
                    <Box>
                        = {calcshowtype ? numberWithCommas((calcamount / price).toFixed(2))
                            : numberWithCommas(Number(price * calcamount).toFixed(2))} {calcshowtype ? 'GVR-BNB' : 'USD'}
                    </Box>
                    <Box>
                ...
                <Box borderRadius={'16px'} mt={'40px'} border={'1px solid black'} padding={'30px'} mx={'20px'} mb={'20px'}>
                    ...
                    <Box fontSize={'28px'} mt={'10px'} fontWeight={'bold'}>${
                        numberWithCommas((calcshowtype ? calcamount * CalculateRate() :
                            calcamount * price * CalculateRate()).toFixed(2))}</Box>
                    <Box mt={'10px'} fontWeight={'bold'}> ~ {
                        calcshowtype ? (calcamount * CalculateRate() / price).toFixed(3) :
                            (calcamount * CalculateRate()).toFixed(3)} GVR-BNB({(CalculateRate() * 100).toFixed(2)}%)</Box>
                </Box>
            ...
            <Box display={'flex'} justifyContent={'center'}>
                <Paper width={'100%'} maxWidth={'600px'} height={'fit-content'}>
                    ...
                    {showdetail ? <Box mt={'20px'} fontSize={'18px'}>
                        ...
                        <Box display={'flex'} >
                            <a href={'https://bscscan.com/address/0xb14173e6E9790C346aCfe9BC02b54AA81841427A'} target={'_blank'}>
                                View Lp Token
                            </a>
                        </Box>
                        <Box display={'flex'} >
                            <a href={'https://pancakeswap.finance/info/pool/0xb14173e6E9790C346aCfe9BC02b54AA81841427A'} target={'_blank'}>
                                See Pair Info
                            </a>
                        </Box>
                    </Box> : ''
